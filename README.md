...
By default, the line height is automatically calculated so that it fits into the defined baseline grid. However, if you need to manually assign a custom line-height to an element, you can do so with an additional (optional) argument:
...
* A number with an arbitrary unit (or unit-less)
* The keyword `inherit`
* The keyword `normal`

You can also completely omit the output of a line-height declaration (because you may want to explicitly inherit the value from another element):
...
With Fluid Typography, you basically define a viewport range in which all the font-sizes are fluid, i.e. the font-sizes a sized according to the viewport size — if the viewport size is small, the font-size is too. If the viewport size is large, so is the font-size.
...
If we use the mixin with `$ms: 2`, this font-size will be again 1.1x bigger than that and so on and on.

##### Changing the ratio on different breakpoints
